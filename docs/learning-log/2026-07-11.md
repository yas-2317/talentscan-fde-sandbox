...
- **Vercel公開URL:** `https://talentscan-fde-sandbox.vercel.app`（本日の作業記録に記載あり。今回の確認ではTLS接続に失敗したため、現在の公開状態は未確認）
...
期待するプロジェクトパスとGitルートが一致しなければ、commitする前に手を止める。独立管理する場合はプロジェクト直下で`git init`し、親側では必要に応じてそのディレクトリを管理対象外にする。ただし、親側からの除外方法と変更内容は今回のSandboxリポジトリからは未確認。
